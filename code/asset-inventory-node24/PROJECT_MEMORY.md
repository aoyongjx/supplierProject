...
## 17. 精准寻源稳定性修复纪要（2026-05-16）

- 本轮核心约束（用户确认）：
  - 核心逻辑不要轻易更改；先稳定，再优化。
  - DB/RAG/WEB 必须有可解释命中，不允许“有命中却展示为0/空白”。
  - 最终输出必须表格化，禁止长段连写难以阅读。

- 已落地修复（前端）：
  - `normalizeLlmMarkdownForRender` 强化，兼容脏格式：`【标题】`、`---##`、`\\n`、`||`、`1.xxx2.yyy` 粘连。
  - 强制表格化四块：`RAG参考`、`明确不足/待核验`、`下一步检索建议`、`建议优先跟进名单`。
  - 修复“可选图表建议”误吞进表格最后一行；避免“图表建议”重复追加。
  - Step4 请求展示增加兜底：缺失计划字段时回退显示 `userInput`，避免 `template/query: -`。

- 已落地修复（后端）：
  - 候选过滤策略改为：`LLM优先 + 空返回回退seed`，修复曾出现的全链路清零（DB/RAG/WEB=0）回归。
  - Google AI Overview 检索扩展：不再只跑1条 query，改为多query + 主机厂拆分补查。
  - AIO 地域语言优先改为中文：`zh-cn/cn/google.com.hk` 优先，海外 profile 作为回退。
