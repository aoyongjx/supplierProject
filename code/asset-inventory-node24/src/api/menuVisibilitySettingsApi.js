...
export async function fetchMenuVisibilitySettings() {
  const response = await requestWithFallback('/api/menu-visibility-settings', {
...
export async function saveMenuVisibilitySettings(payload) {
  const response = await requestWithFallback('/api/menu-visibility-settings', {
    method: 'PUT',
    headers: {
